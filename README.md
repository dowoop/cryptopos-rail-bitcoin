...
**Not on PyPI yet**, so `pip install cryptopos-rail-bitcoin` does not resolve — the repository is the
...
configuration = {
    ...
    # "transport": my_client,                           # optional; anything with a
    #                                                   # .get(url, timeout, max_bytes)
}
```
